...
do
    ...
    echo "Running: mysqldump --defaults-file=$DEFAULTS_FILE $db | $BKUP_BIN $BKUP_COMPRESSION > $backupDir/$backupFile"
    ...
    mysqldump --defaults-file=$DEFAULTS_FILE "$db" | $BKUP_BIN $BKUP_COMPRESSION > "$backupDir/$backupFile"
    echo
done
